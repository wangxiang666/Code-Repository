...
(function (_, joint) {
  ...
  App.MainView = joint.mvc.View.extend({
    ...
    initializeSelection: function () {
      ...
      this.selection = new joint.ui.Selection({ paper: this.paper, handles: App.config.selection.handles, useModelGeometry: true });
      ...
      this.paper.on(
        {
          ...
          "element:pointerdblclick": function (cellView) {
            ...
            // if (type !== "standard.Polygon" && type !== "bpmn.Pool") {
            if (type !== "standard.Rectangle" && type !== "bpmn.Pool") {
          ...
          "element:pointerdown": function (elementView, evt) {
            ...
            this.upStore({ id: elementView.model.id, type: elementView.model.attributes.type }, "upData");
          ...
          "link:pointerdown": function (elementView) {
            console.log("element:pointerdown ------  在链接上按下指针时触发。", elementView.model.id);
            this.upStore({ id: elementView.model.id, type: elementView.model.attributes.type }, "upData");
          },
    ...
    onSelectionChange: function () {
      ...
      if (collection.length === 1) {
        ...
        collection.each(function (cell) { selection.createSelectionBox(cell); });
      }
    },
        ...
        _.extend(
          {
            ...
            validateInput: function (el, path, type, inspector) {
              ...
              let inspectorList = JSON.parse(
                sessionStorage.getItem("inspectorList")
              );
              let state = true;
              ...
              //监听修改的内容
              var $el = $(el), html; let value = inspector.parse(type, inspector.getFieldValue(el, type), el);
              ...
              if (state) {
                ...
                if (!inspectorList) {
                  inspectorList = {};
                  inspectorList[cell.id] = { id: cell.id, type: cell.attributes.type, position: cell.attributes.position, ip: value };
                } else {
                  inspectorList[cell.id] = { id: cell.id, type: cell.attributes.type, position: cell.attributes.position, ip: value };
                }
      ...
      this.graph.on(
        {
          ...
          add: function (cell) {
            ...
            if (cell.attributes.type === "app.Link" || cell.attributes.type === "standard.Rectangle" || cell.attributes.type === "bpmn.Pool") {
              return;
            }
            ...
            let typeDataType = Object.prototype.toString
              .call(typeDataText)
              .slice(8, -1);
            if (!inspectorList) {
              inspectorList = {};
    ...
    openAsSVG: function () {
      ...
      paper.hideTools().toSVG(function (svg) {
        new joint.ui.Lightbox({ image: 'data:image/svg+xml,' + encodeURIComponent(svg), downloadable: true, fileName: '资产拓扑' }).open();
        paper.showTools();
      }, {
    ...
    layoutDirectedGraph: function (e) {
      ...
      let screenWidth = document.body.clientWidth, screeHeight = document.body.clientHeight;
      this.paperScroller.transitionToPoint((screenWidth - 400) / 2, (screeHeight - 200) / 2);
      // this.paperScroller.scrollToContent();
      //  // 包围图形中所有单元格的边界框
      //  let getLinks = this.graph.getLinks();
      //  let bbox = this.graph.getBBox().inflate(10);
      // let getBBox = this.graph.getBBox();
      // let getAncestors = this.graph.getAncestors();
      // let getAncestors = new joint.dia.link.getAncestors()
      //  console.log("bbox", bbox);
      // console.log("getBBox", getBBox);
      // console.log("getAncestors", getAncestors);
      //  console.log("getLinks", getLinks);
      //  this.paperScroller.transitionToPoint(bbox.x, bbox.y);
    },
    saveData: function () {
    ...
    clickClear: function () {
      const num = new Date().getTime() + "xxxxxx".replace(/[xy]/g, function () {
    ...
    upStore: function (obj, type) {
      // let editingState = sessionStorage.getItem("editingState");
      // if(editingState == 'false') return; //判断是否编辑
      // console.log("upStore方法：", obj, type, editingState);
      ...
      //数组去重复
      // data[type] = _.uniqWith(data[type], _.isEqual);
